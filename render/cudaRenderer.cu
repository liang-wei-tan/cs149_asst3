...
__global__ void kernelRenderCircleBoundingRectanglesAndInit(CircleParams* cudaDeviceCircleParams, int* cudaLatestDependency) {
    ...
    circleParams.screenMinX = screenMinX;
    circleParams.screenMaxX = screenMaxX;
...
__global__ void kernelRenderCirclesPerPixel() {
    ...
    sOut[linearThreadIndex] = 0;
    scratch[linearThreadIndex] = 0;
    queue[linearThreadIndex] = 0;
    scratch[linearThreadIndex + BLOCKSIZE] = 0;
    __syncthreads();
